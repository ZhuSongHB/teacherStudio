	...
	<div>
		...
		<el-collapse v-model="activeName" id="cur" accordion @change="change(activeName)" v-loading="uploadLoading" element-loading-text="上传附件中......">
			<el-collapse-item v-for="(item, i) in member" :key="i" :name="i" :title="item.studio_name">
				...
				<el-form ref="sub_data" :model="sub_data" label-width="160px" :rules="rules">
					...
					<el-form-item label="受理单位" class="desc" prop="unit_id">
						<el-select v-model="sub_data.unit_id" style="width:330px;" placeholder="受理单位请选择置换的学分所在的开课单位">
					...
					<el-form-item label="申请理由" prop="reason">
						<el-input v-model="sub_data.reason" type="textarea" placeholder="抵换方式可从工作内容时长、难度、工作量，成果如何抵换学分、材料提交方式与成绩认定方式等进行介绍" style="width:80%;"></el-input>
					</el-form-item>
					<!-- 上传 -->
					...
					<div class="add">
						...
						<el-input placeholder="学分类型" v-model.trim="tcrData.credit_type"></el-input>
						<el-input placeholder="学分数" v-model.trim="tcrData.credit"></el-input>
					...
					<el-table :data="tableData" stripe style="width: 80%">
						...
						<el-table-column prop="credit_type" label="学分类型"></el-table-column>
						<el-table-column prop="credit" label="学分数"></el-table-column>
			...
			<el-dialog title="修改" :visible.sync="dialogVisible" width="30%">
				...
				<div>课程名称</div>
				<el-input placeholder="学分类型" v-model.trim="changeData.credit_type"></el-input>
				<div>学分数</div>
				<el-input placeholder="学分数" v-model.trim="changeData.credit"></el-input>
				<div>学生工作情况说明</div>
...
<script>
	import { creditReplaceApplySubmit, getCreditProjectCode } from "../../../network/api";
	export default {
		props: ["member", "partyUnitData"],
		watch: {
			member() {
				getCreditProjectCode({ studio_scode: this.member[0].studio_scode }).then(res => {
		...
		methods: {
			...
			// 添加到下列表格中
			addTableData() {
				if (this.tcrData.student_name != "" && this.tcrData.credit != "" && this.tcrData.student_id != "" && this.tcrData.credit_type != "" && this.tcrData.explain != "") {
			...
			change(i) {
				...
				this.tableData = [];
				if (this.member[i]) {
					getCreditProjectCode({ studio_scode: this.member[i].studio_scode }).then(res => {
						this.projectCodeArr = res.data.data;
					});
				}
			},
