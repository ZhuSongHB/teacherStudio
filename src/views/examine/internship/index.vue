	...
	<div>
		...
		<el-collapse v-model="activeName" id="cur" accordion @change="change(activeName)" v-loading="uploadLoading" element-loading-text="上传附件中......">
			<el-collapse-item v-for="(item, i) in member" :key="i" :name="i" :title="item.studio_name">
				...
				<el-form ref="sub_data" :model="sub_data" label-width="160px" :rules="rules">
					...
					<el-form-item label="申请理由" prop="reason">
						<el-input v-model="sub_data.reason" type="textarea" placeholder="介绍学生在工作室中的成果，承担实习的方案，材料提交方式等" style="width:80%;"></el-input>
					</el-form-item>
					...
					<div class="add">
						...
						<el-select v-model.trim="tcrData.type" placeholder="实习类型">
					...
					<el-table :data="tableData" stripe style="width: 80%">
						...
						<el-table-column prop="type" label="实习类型">
							<template slot-scope="scope">
								{{ options1[scope.row.type] ? options1[scope.row.type - 1]["type"] : "" }}
							</template>
						</el-table-column>
			...
			<el-dialog title="修改" :visible.sync="dialogVisible" width="30%">
				...
				<div>实习类型</div>
				<el-select v-model.trim="changeData.type" placeholder="实习类型">
					<el-option v-for="item in options1" :label="item.type" :value="item.id" :key="item.id"></el-option>
				</el-select>
				<div>指导教师</div>
...
<script>
	import { assumePracticeApplySubmit, practiceSchemeGetTypeList, getPracticeProjectCode } from "../../../network/api";
	export default {
		...
		methods: {
			...
			btn_sub(i, index) {
				...
				if (!key) {
					this.$message({
						type: "warning",
						message: "承担实习基础信息填写不完整!",
					});
					return;
				}

				this.$confirm("确认提交承担实习申请吗?", "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning",
				}).then(() => {
					assumePracticeApplySubmit(obj)
						.then(res => {
							res = res.data;
							if (res.code === 1) {
								// 提交文件
			...
			// 添加到下列表格中
			addTableData() {
				if (this.tcrData.student_name != "" && this.tcrData.student_id != "" && this.tcrData.type != "" && this.tcrData.teacher_name != "" && this.tcrData.explain != "") {
