<template>
	<div>
		...
		<el-collapse v-model="activeName" accordion @change="change" v-loading="uploadLoading" element-loading-text="上传附件中......">
			<el-collapse-item v-for="(item, i) in member" :key="i" :name="i" :title="item.studio_name">
				...
				<el-form ref="curriculum" :model="curriculum" label-width="170px" :rules="rules">
					...
					<div>
						...
						<el-form-item label="抵换课程证明材料说明" class="desc" prop="material">
							<el-input type="textarea" v-model="curriculum.material" placeholder="抵换课程证明材料说明" class="textarea"></el-input>
						</el-form-item>
						<!-- 上传 -->
					...
					<div class="test">
						<el-divider>抵换课程录入</el-divider>
						<div class="add">
							<el-input placeholder="抵换的课程或课程组" v-model.trim="tcrData.course"></el-input>
						...
						<el-table :data="tableData" stripe style="width: 600px">
							<el-table-column prop="course" label="课程或课程组"></el-table-column>
			...
			<el-dialog title="修改" :visible.sync="dialogVisible" width="30%">
				<div>所抵换的课程或课程组</div>
				<el-input placeholder="抵换的课程或课程组" v-model.trim="changeData.course"></el-input>
				<div>抵换方式</div>
...
<script>
	import { courseReplaceSchemeApplySubmit } from "../../../network/api";
	export default {
		...
		data() {
			return {
				...
				rules: {
					...
					material: [{ required: true, message: "请输入抵换课程证明材料说明", trigger: "blur" }],
				},
				activeName: [0],
				// 工作室的基本数据
				curriculum: {
		...
		methods: {
			...
			registerFormSubmit(i, index) {
				...
				this.$confirm("确认提交课程置换方案申请吗?", "提示", {
					...
					courseReplaceSchemeApplySubmit(obj)
						.then(res => {
							res = res.data;
							if (res.code === 1) {
								// 提交文件
			...
			// 添加到下列表格中
			addTableData() {
				if (this.tcrData.curriculum != "" && this.tcrData.way != "") {
