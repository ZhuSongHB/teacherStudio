	...
	<div>
		...
		<el-dialog :title="studioData.studio_name" :visible.sync="dialogVisible" width="60%" class="box">
			<el-form ref="studioData" :model="studioData" label-width="150px">
				...
				<el-divider>抵换课程</el-divider>
				<div class="addTeacher">
					<el-table :data="scheme" style="width: 100%">
						<el-table-column prop="course" label="所抵换的课程或课程组"></el-table-column>
...
<script>
	import { courseSchemeGetList, courseSchemeGetTable, courseSchemeCancel } from "../../../network/api";
	...
	export default {
		...
		methods: {
			...
			handleEdit(tableData) {
				courseSchemeGetTable({ record_id: tableData.id }).then(res => {
			...
			handleDelete(tableData) {
				// 提交
				this.$confirm("确认撤销课程置换方案申请吗?", "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning",
				}).then(() => {
					courseSchemeCancel({ record_id: tableData.id })
						.then(res => {
							this.loading = true;
							if (res.data.code == 1) {
								const teacher_id = sessionStorage.getItem("id");
								courseSchemeGetList({ teacher_id: teacher_id }).then(res => {
									this.loading = false;
