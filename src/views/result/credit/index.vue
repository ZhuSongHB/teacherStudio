...
<script>
	import { creditReplaceApplyGetList, creditReplaceApplyGetTable, creditReplaceApplyCancel } from "../../../network/api";
	...
	export default {
		...
		methods: {
			...
			handleEdit(tableData) {
				creditReplaceApplyGetTable({ record_id: tableData.id }).then(res => {
			...
			handleDelete(tableData) {
				// 提交
				this.$confirm("确认撤销工作室学分置换申请吗?", "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning",
				}).then(() => {
					creditReplaceApplyCancel({ record_id: tableData.id })
						.then(res => {
							this.loading = true;
							if (res.data.code == 1) {
								const teacher_id = sessionStorage.getItem("id");
								creditReplaceApplyGetList({ teacher_id: teacher_id }).then(res => {
									this.loading = false;
