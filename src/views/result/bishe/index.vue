...
<script>
	import { assumeThesisApplyGetList, assumeThesisApplyGetTable, assumeThesisApplyCancel } from "../../../network/api";
	...
	export default {
		...
		methods: {
			...
			handleEdit(tableData) {
				assumeThesisApplyGetTable({ record_id: tableData.id }).then(res => {
			...
			handleDelete(tableData) {
				...
				this.$confirm("确认承担实习申请吗?", "提示", {
					...
					assumeThesisApplyCancel({ record_id: tableData.id })
						.then(res => {
							this.loading = true;
							if (res.data.code == 1) {
								const teacher_id = sessionStorage.getItem("id");
								assumeThesisApplyGetList({ teacher_id: teacher_id }).then(res => {
									this.loading = false;
