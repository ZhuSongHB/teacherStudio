...
<script>
	import { courseReplaceApplyGetList, courseReplaceApplyGetTable, courseReplaceApplyCancel } from "../../../network/api";
	...
	export default {
		...
		methods: {
			...
			handleEdit(tableData) {
				courseReplaceApplyGetTable({ record_id: tableData.id }).then(res => {
			...
			handleDelete(tableData) {
				// 提交
				this.$confirm("确认撤销课程置换申请吗?", "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning",
				}).then(() => {
					courseReplaceApplyCancel({ record_id: tableData.id })
						.then(res => {
							this.loading = true;
							if (res.data.code == 1) {
								const teacher_id = sessionStorage.getItem("id");
								courseReplaceApplyGetList({ teacher_id: teacher_id }).then(res => {
									this.loading = false;
