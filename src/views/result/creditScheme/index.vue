	...
	<div>
		...
		<el-dialog :title="studioData.studio_name" :visible.sync="dialogVisible" width="60%" class="box">
			<el-form ref="studioData" :model="studioData" label-width="150px">
				...
				<el-form-item label="证明材料" class="desc">
					<el-input v-model.trim="studioData.material" class="textarea" disabled type="textarea"></el-input>
				</el-form-item>
				...
				<el-divider>抵换学分</el-divider>
				<div class="addTeacher">
					<el-table :data="scheme" style="width: 100%">
						<el-table-column prop="credit_type" label="所抵换的课程或课程组"></el-table-column>
						<el-table-column prop="credit" label="分数"></el-table-column>
...
<script>
	import { creditSchemeGetList, creditSchemeGetTable, creditSchemeCancel } from "../../../network/api";
	...
	export default {
		...
		methods: {
			...
			handleEdit(tableData) {
				creditSchemeGetTable({ record_id: tableData.id }).then(res => {
					this.studioData = res.data.data;
					this.dialogVisible = true;
					const scheme = [];
					for (const item of res.data.data.scheme) {
						scheme.push(item);
					}
					this.scheme = scheme;
					// this.scheme
			...
			handleDelete(tableData) {
				// 提交
				this.$confirm("确认撤销学分置换分案申请吗?", "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning",
				}).then(() => {
					creditSchemeCancel({ record_id: tableData.id })
						.then(res => {
							this.loading = true;
							if (res.data.code == 1) {
								const teacher_id = sessionStorage.getItem("id");
								creditSchemeGetList({ teacher_id: teacher_id }).then(res => {
									this.loading = false;
